...
original <- read.csv("Original_Dataset_Stress.csv")
synthetic <- read.csv("Synthetic_Dataset_Stress.csv")
...
print(permanova_result) # p=0.556, no statistical difference between original and synthetic datasets

# can ML model distinguish between original and synthetic?
linearly_independent$Group <- factor(c(rep(0, nrow(original)), rep(1, nrow(synthetic))))
set.seed(42)
rf <- randomForest(Group ~ ., data=linearly_independent) # OOB estimate of  error rate: 52.04%
linearly_independent <- linearly_independent[, 2:22]
set.seed(42)
rf <- randomForest(Group ~ ., data=linearly_independent) # OOB estimate of  error rate: 52.04% - same
