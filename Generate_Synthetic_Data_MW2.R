...
#########################################################################################################################################
data <- read.csv("EEG_Dataset_MW2.csv")
temp <- data
temp$Index <- NULL
...
temp <- cbind(alpha_frontal, beta_frontal, delta_frontal, theta_frontal, gamma_frontal,
              ...
              temp$Difficulty, temp$HR, temp$HRV, temp$Test, temp$Stress)
temp <- as.data.frame(temp)
names(temp)[26] <- "Difficulty"
names(temp)[27] <- "HR"
names(temp)[28] <- "HRV"
names(temp)[29] <- "Test"
...
temp$Medium <- 0
temp$High <- 0
temp[temp$Difficulty == 1,"Low"] <- 1
temp[temp$Difficulty == 2,"Medium"] <- 1
temp[temp$Difficulty == 3,"High"] <- 1
temp$Test <- NULL
names(temp) <- c("Alpha Frontal", "Beta Frontal", "Delta Frontal", "Theta Frontal", "Gamma Frontal",
                 ...
                 "Difficulty", "HR", "HRV","Stress", "Low", "Medium", "High")
...
write.csv(temp, "Original_Dataset_MW2.csv", row.names = FALSE)
write.csv(synthetic_dataset, "Synthetic_Dataset_MW2.csv", row.names = FALSE)
