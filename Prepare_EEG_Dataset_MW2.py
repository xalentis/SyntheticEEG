...
subjects = [1, 2]
hr_data = pd.read_parquet(os.path.join("MentalWorkloadEEG", "data_flight_simulator", "ecg", "ecg_hr.parquet"), engine="pyarrow")
ibi_data = pd.read_parquet(os.path.join("MentalWorkloadEEG", "data_flight_simulator", "ecg", "ecg_ibi.parquet"), engine="pyarrow")
eeg_data = pd.read_parquet(os.path.join("MentalWorkloadEEG", "data_flight_simulator", "eeg", "eeg.parquet"), engine="pyarrow")

for subject in subjects:
    print("Processing: " + str(subject))
    for flight in range(1,6):
        hr = hr_data.loc[(hr_data["subject"] == subject) & (hr_data["flight"] == flight)]
        hr = hr["hr"].values
        ibi = ibi_data.loc[ibi_data["subject"] == subject]
        subset = eeg_data.loc[(eeg_data["subject"] == subject)  & (eeg_data["flight"] == flight)]
        if subset.shape[0] > 0 and hr.shape[0] > 0 and ibi.shape[0] > 0: # if we have this flight
            ...
            data_filtered = data_raw.copy().filter(l_freq=1, h_freq=45)
            ...
            for epoch in range(0, len(epochs)):
                ...
                subset["Test"] = str(flight)
                subset["Difficulty"] = "2" # medium
                if flight == 1:
                    subset["Difficulty"] = "1" # easy
                if flight == 3:
                    subset["Difficulty"] = "3" # hard
...
final_dataset.to_csv("EEG_Dataset_MW2.csv", index=False)